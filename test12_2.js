const splitIntoSentences = (text) => {
    ...
    const abbreviations = ['Mr','Mrs','Ms','Miss','Dr','Prof','St','Jr','Sr','Rev','Lt','Capt','Col','Gen','Sgt','Cpl','Pvt','Gov','GS','PGS','TS','ThS','BS','KS','LS','Tp','TP'];

    const isUpperishStart = (str) => /^["'"「『\-\s]*[\p{Lu}]/u.test(str);
    const isUpperishStartAfterSpace = (str) => /^["'"\u300c\u300e\-]*\s+["'"\u300c\u300e\-\s]*[\p{Lu}]/u.test(str);
    const isAbbreviation = (buffer) => {
        const trimmed = buffer.trim().replace(/[.!?…]+$/, '');
        const words = trimmed.split(/[\s\.]+/);
        return abbreviations.includes(words[words.length - 1]);
    };
    const countChar = (str, char) => { let c=0; for(let i=0;i<str.length;i++) if(str[i]===char) c++; return c; };
    ...
    const WEIGHTS = {
        ...
        ENDS_EXCLAIM:          +1,
        NO_OUTER_WORDS:        +1,
        HAS_TRAILING_QUOTE:    +1,
        ENDS_DOT:              +1,
        NEXT_OPEN_QUOTE:       +1,
        ENDS_QUESTION:         +1,

        // FIX 1: SHORT_QUOTE_PATTERN veto
        SHORT_QUOTE_PATTERN:   -10,
        // FIX 2: HAS_INTERNAL_SENTENCE mạnh hơn
        HAS_INTERNAL_SENTENCE: -7,
        // FIX 3: NEXT_COMMA_SEMICOLON mạnh hơn
        NEXT_COMMA_SEMICOLON:  -7,
        // FIX 4: QUOTE_NARRATION mới
        QUOTE_NARRATION:       -4,
        // FIX 5: Tách ATTACHED_WORD thành 2
        ATTACHED_LOWER:        -4,  // lowercase attached → block
        ATTACHED_WORD:         -4,  // giữ cho backward compat (dùng trong quote-boundary)
        IS_ABBREVIATION:      -10,
    };
    ...
    const computeScore = (signals) => {
        ...
        for (const [key, val] of Object.entries(signals)) {
            if (val && WEIGHTS[key] !== undefined) score += WEIGHTS[key];
        }
        return score;
    };
    ...
    for(const [open,close] of Object.entries(specialPairs)) {
        if(countChar(text,open)!==countChar(text,close)) { unbalanced[open]=true; unbalanced[close]=true; }
    }
    ...
    const splitSegment = (seg) => {
        ...
        while(i < seg.length) {
            ...
            if(allQuotes.includes(ch)) {
                ...
                if(ch==='"') {
                    if(openQuoteIndices.has(i)) quoteLevel++;
                    else if(closeQuoteIndices.has(i)) quoteLevel=Math.max(0,quoteLevel-1);
                } else if(openQuotes.includes(ch)) { if(!unbalanced[ch]) quoteLevel++; }
                else if(closeQuotes.includes(ch)) { if(!unbalanced[ch]) quoteLevel=Math.max(0,quoteLevel-1); }
                current+=ch; i++;
                ...
                if(quoteLevel===0 && !unbalanced[ch]) {
                    ...
                    if(nextNonSpaceMatch) {
                        ...
                        if(endingPunctMatch && !canSplitQuote && openQuotes.includes(nextChar)) {
                            let reallyOpen=true;
                            if(nextChar==='"') { const ni=i+rest.indexOf('"'); if(!openQuoteIndices.has(ni)) reallyOpen=false; }
                        ...
                        if(endingPunctMatch && canSplitQuote) {
                            ...
                            if(!isAbbreviation(current)) {
                                let isAttachedCloseQuote=closeQuotes.includes(rest[0]);
                                if(rest[0]==='"'&&openQuoteIndices.has(i)) isAttachedCloseQuote=false;
                                ...
                                if(!isAttachedWord && ((isUpperishStart(rest)&&!isJustEllipsis)||allQuotes.includes(nextChar))) {
                                    let nextComma=false;
                                    if(!allQuotes.includes(nextChar)) {
                                        const m=rest.match(/([.,;:!?…，；：])/);
                                        if(m&&[',',';',':','，','；','：'].includes(m[1])) nextComma=true;
                                    }

                                    const isNextOpenQuote = allQuotes.includes(nextChar);
                                    const endsExclaim = /!/.test(punct);
                                    const endsQuestion = /\?/.test(punct) && !endsExclaim;

                                    const signals = {
                                        IS_ABBREVIATION: isAbbreviation(current),
                                        NEXT_UPPER: isUpperishStart(rest) && !isJustEllipsis,
                                        HAS_SPACE: /^\s/.test(rest),
                                        HAS_OUTER_WORDS: hasOuterWords,
                                        NEXT_IS_EMPTY: rest.trim().length===0,
                                        STARTED_WITH_QUOTE: startedWithQuote,
                                        ENDS_EXCLAIM: endsExclaim,
                                        ENDS_QUESTION: endsQuestion,
                                        ENDS_DOT: /^\.+$/.test(punct) && punct.length===1,
                                        NO_OUTER_WORDS: !hasOuterWords,
                                        HAS_TRAILING_QUOTE: true,
                                        NEXT_OPEN_QUOTE: isNextOpenQuote,
                                        SHORT_QUOTE_PATTERN: /^["'"「『](?:\s+|…+|\.+)[^.!?…]{1,15}[.!?…]+[""」』]*$/.test(current.trim()),
                                        HAS_INTERNAL_SENTENCE: hasInternalSentence && !isNextOpenQuote,
                                        NEXT_COMMA_SEMICOLON: nextComma,
                                        ATTACHED_WORD: false,
                                        ATTACHED_LOWER: false,
                                        ELLIPSIS_ATTACHED: false,
                                        // FIX 4: QUOTE_NARRATION - thuần quote, có space, next không phải open quote, không phải !/?
                                        QUOTE_NARRATION: !hasOuterWords && /^\s/.test(rest) && !isNextOpenQuote && !endsExclaim && !endsQuestion,
                                    };
            ...
            if(quoteLevel===0 && /[.!?…]/.test(ch)) {
                let punct='';
                while(i<seg.length&&/[.!?…]/.test(seg[i])) { punct+=seg[i]; i++; }
                const tempCurrent=current+punct;
                if(!isAbbreviation(tempCurrent)) {
                    let trailingQuotes='';
                    while(i<seg.length) {
                        const nc=seg[i];
                        if(nc==='"'&&openQuoteIndices.has(i)) break;
                        if(/["'"」』]/.test(nc)) { trailingQuotes+=nc; i++; } else break;
                    }
                    ...
                    if(isUnicodeEllipsisOnly) {
                        ...
                        else if(isUpperishStart(rest) && !current.endsWith(' ')) upperCheck=true;
                    } else {
                        upperCheck=isUpperishStart(rest);
                    }

                    let isAttachedCloseQ=closeQuotes.includes(rest[0]);
                    if(rest[0]==='"'&&openQuoteIndices.has(i)) isAttachedCloseQ=false;
                    const attachedWord=/^[\p{L}\p{N}]/u.test(rest)||isAttachedCloseQ;
                    // FIX 5: Phân biệt uppercase vs lowercase attached
                    const attachedLower = attachedWord && !(/^[\p{Lu}]/u.test(rest.trimStart()));

                    let nextComma=false;
                    if(trailingQuotes.length>0) {
                        const nm=rest.match(/^\s*(.)/);
                        if(nm&&!allQuotes.includes(nm[1])) {
                            const m=rest.match(/([.,;:!?…，；：])/);
                            if(m&&[',',';',':','，','；','：'].includes(m[1])) nextComma=true;
                        }
                    }

                    const isNextOpenQuote = /^["'"「『]/.test(rest.trimStart());
                    const endsExclaim = /!/.test(punct);
                    const endsQuestion = /\?/.test(punct) && !endsExclaim;

                    const signals = {
                        IS_ABBREVIATION: isAbbreviation(fullCurrent),
                        NEXT_UPPER: upperCheck,
                        HAS_SPACE: /^\s/.test(rest),
                        HAS_OUTER_WORDS: hasOuterWords,
                        NEXT_IS_EMPTY: rest.trim().length===0,
                        STARTED_WITH_QUOTE: startedWithQuote,
                        ...
                        ENDS_DOT: /^\.+$/.test(punct) && punct.length===1,
                        NO_OUTER_WORDS: !hasOuterWords,
                        HAS_TRAILING_QUOTE: trailingQuotes.length>0,
                        NEXT_OPEN_QUOTE: isNextOpenQuote,
                        SHORT_QUOTE_PATTERN: /^["'"「『](?:\s+|…+|\.+)[^.!?…]{1,15}[.!?…]+[""」』]*$/.test(fullCurrent.trim()),
                        HAS_INTERNAL_SENTENCE: false,
                        NEXT_COMMA_SEMICOLON: nextComma,
                        ATTACHED_WORD: false,
                        ATTACHED_LOWER: attachedLower,  // FIX 5: lowercase attached thôi
                        ELLIPSIS_ATTACHED: isUnicodeEllipsisOnly && /^[\p{Lu}]/u.test(rest.trimStart()) && !/^\s/.test(rest),
                        QUOTE_NARRATION: false,
                    };

                    if(/^["'"「『\-\s.!?…]+$/.test(fullCurrent.trim())) {
                        current=fullCurrent; continue;
                    }
    ...
    return splitSegment(text).map(s=>s.trim()).filter(s=>s.replace(/["""「」『』'.,!?…\-\s]/g,'').length>0);
};
