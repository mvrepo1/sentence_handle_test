...
//             // --- FIX TEST 58: Phát hiện ngoặc đóng bị đặt lệch sau khoảng trắng ---
//             let isMisplacedClosing = false;
//             if (inQuote && /\s/.test(prev)) {
//                 let idx = i - 1;
//                 // Duyệt ngược bỏ qua khoảng trắng để check dấu câu trước đó
...
//                 isClosing = true; // Ép về ngoặc đóng để kích hoạt cơ chế chuẩn hóa khoảng trắng
...
//             if (isOpening) {
//                 inQuote = true;
//                 // FIX TEST 58: Không bơm khoảng trắng giữa 2 ngoặc mở đứng cạnh nhau (VD: ""Chắc là)
//                 if (normalizedText.length > 0) {
//                     const lastChar = normalizedText[normalizedText.length - 1];
//                     // Chỉ bơm khoảng trắng nếu liền trước là chữ/số/dấu câu, HOẶC nếu nó vừa là 1 ngoặc ĐÓNG
...
//                     // Nếu trước và sau dấu chấm đều là số
...
//                     // Thêm dấu ? ngay sau khối bắt buộc
...
//   // Đếm đối xứng ngoặc đơn & ngoặc vuông
...
//     let current = '';
//     let quoteLevel = 0;
//     let parenthesisLevel = 0; // Trạng thái ngoặc đơn ()
//     let squareBracketLevel = 0; // Trạng thái ngoặc vuông []
//     let hasOuterWords = false;
//     let startedWithQuote = false;
//     let i = 0;
...
//       // Cập nhật trạng thái cấp độ ngoặc đơn & vuông
...
//                   // Chỉ cắt câu khi không nằm trong bất kỳ ngoặc nào
...
//       // CHỈ XỬ LÝ NGẮT CÂU KHI ĐANG Ở CẤP ĐỘ NGOÀI CÙNG (quoteLevel == 0 && parenthesisLevel == 0 && squareBracketLevel == 0)
//       if (quoteLevel === 0 && parenthesisLevel === 0 && squareBracketLevel === 0 && /[.!?…]/.test(ch)) {

//         // FIX LỖI SỐ THẬP PHÂN (VD: 59.2)
//         if (ch === '.' && i > 0 && i < seg.length - 1) {
//           const prevChar = seg[i - 1];
//           const nextChar = seg[i + 1];
//           if (/\p{N}/u.test(prevChar) && /\p{N}/u.test(nextChar)) {
