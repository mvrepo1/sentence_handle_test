const splitIntoSentences = (text) => {
    ...
    // Đã fix lại ngoặc lệch (Sử dụng đúng “ và ”)
    ...
    // =========================================================
    // SCORING SYSTEM (Đã re-balance)
    // =========================================================
    const WEIGHTS = {
        // Tín hiệu tích cực -> Xúi giục cắt (Max khoảng 10đ)
        NEXT_UPPER: +3,
        HAS_SPACE: +3,
        HAS_OUTER_WORDS: +2,
        NEXT_IS_EMPTY: +2,
        ...
        // Tín hiệu tiêu cực -> Ép buộc không cắt (Hard Veto)
        ELLIPSIS_ATTACHED: -10, // Pause giữa câu
        HAS_INTERNAL_SENTENCE: -10, // Bên trong ngoặc đã có câu dài
        SHORT_QUOTE_PATTERN: -10, // Ngoặc quá ngắn 
        NEXT_COMMA_SEMICOLON: -10, // Dấu phẩy ngay phía sau
        ATTACHED_WORD: -10, // Bám dính chữ liền mạch
        IS_ABBREVIATION: -10, // Viết tắt
    };

    const SPLIT_THRESHOLD = 4;

    const computeScore = (signals, debug = false) => {
        let score = 0;
        ...
        while (i < seg.length) {
                ...
                if (quoteLevel === 0 && !unbalanced[ch]) {
                    ...
                    if (nextNonSpaceMatch) {
                        ...
                        if (endingPunctMatch && !canSplitQuote && openQuotes.includes(nextChar)) {
                            let reallyOpen = true;
                            if (nextChar === '"') {
                                const ni = i + rest.indexOf('"');
                                if (!openQuoteIndices.has(ni)) reallyOpen = false;
                            }
                        ...
                        if (endingPunctMatch && canSplitQuote) {
                            ...
                            const signals = {
                                ...
                                ENDS_EXCLAIM: /!/.test(punct),
                                ENDS_QUESTION: /\?/.test(punct) && !/!/.test(punct),
                                ENDS_DOT: /^\.+$/.test(punct) && punct.length === 1,
                                NO_OUTER_WORDS: !hasOuterWords,
                                HAS_TRAILING_QUOTE: true,
                                NEXT_OPEN_QUOTE: allQuotes.includes(nextChar),
                                SHORT_QUOTE_PATTERN: /^["'“「『](?:\s+|…+|\.+)[^.!?…]{1,15}[.!?…]+["”」』]*$/.test(current.trim()),
                                HAS_INTERNAL_SENTENCE: hasInternalSentence && !allQuotes.includes(nextChar),
                                NEXT_COMMA_SEMICOLON: nextComma,
                                ATTACHED_WORD: isAttachedWord,
            ...
            if (quoteLevel === 0 && /[.!?…]/.test(ch)) {
                ...
                if (!isAbbreviation(tempCurrent)) {
                    ...
                    // Scope fix: Chỉ check attached word & next comma nếu có trailingQuotes
                    let nextComma = false;
                    let attachedWord = false;
                    ...
                    if (trailingQuotes.length > 0) {
                        ...
                        attachedWord = /^[\p{L}\p{N}]/u.test(rest) || isAttachedCloseQ;
                    ...
                    const signals = {
                        ...
                        NEXT_OPEN_QUOTE: /^["'“「『]/.test(rest.trimStart()),
                        SHORT_QUOTE_PATTERN: /^["'“「『](?:\s+|…+|\.+)[^.!?…]{1,15}[.!?…]+["”」』]*$/.test(fullCurrent.trim()),
                        HAS_INTERNAL_SENTENCE: false,
                        NEXT_COMMA_SEMICOLON: nextComma,
                        ATTACHED_WORD: attachedWord,
                        ELLIPSIS_ATTACHED: isUnicodeEllipsisOnly && /^[\p{Lu}]/u.test(rest.trimStart()) && !/^\s/.test(rest) && current.endsWith(' '),
                    };
