const splitIntoSentences = (text) => {
  ...
  const splitSegment = (seg) => {
    ...
    // if (ignoreStraightQuotes && seg.trimStart().startsWith('"')) {
    //   const firstPunctMatch = seg.match(/^"\s*[^.!?…]{1,15}[.!?…]/);
    //   if (firstPunctMatch) {
    //     const stripped = seg.trimStart().slice(1);
    //     const parts = splitSegment(stripped);
    //     if (parts.length > 1) {
    //       // Merge câu đầu (ngắn) vào câu thứ hai
          ...
          if (nextNonSpaceMatch) {
            ...
            if (endingPunctMatch && (!hasOuterWords || startedWithQuote)) {
              ...
              if (!hasInternalSentence && !isAbbreviation(current)) {
                // SỬA LỖI: Kiểm tra xem ký tự tiếp theo có bị dính liền vào ngoặc kép không
      ...
      if (quoteLevel === 0 && /[.!?…]/.test(ch)) {
        ...
        if (!isAbbreviation(tempCurrent)) {
          ...
          if (trailingQuotes.length > 0) {
            ...
            // SỬA LỖI: Ngăn tách nếu phía sau là một chữ/số dính liền không có dấu cách
...
const tests = [
  ...
  },
  {
    ...
    expected: [
      '" Bất quá. Khi tiến vào sa mạc, Hắn đã tích trữ nguồn nước,..Lần trước chuẩn bị, đã dùng một thời gian, nhìn chỗ chứa nước trong giới chỉ, Tiêu Viêm thở dài một hơi.',
